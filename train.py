...
parser.add_argument('--trainpath',default='../DTUdataset/dtu_training/mvs_training/dtu/', help='train datapath')
parser.add_argument('--testpath',default='../DTUdataset/dtu_training/mvs_training/dtu/', help='test datapath')
...
parser.add_argument('--logdir', default='./checkpoints_stage2', help='the directory to save checkpoints/logs')
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    loss, depth_loss = model_loss(outputs,refine_depth_map, depth_gt_ms, mask_ms, dlossw=[float(e) for e in args.dlossw.split(",") if e])
...
def test_sample_depth(model, model_loss, sample, args):
    ...
    outputs,depth_loss = model_eval(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]

    loss, depth_loss = model_loss(outputs,depth_loss, depth_gt_ms, mask_ms, dlossw=[float(e) for e in args.dlossw.split(",") if e])
