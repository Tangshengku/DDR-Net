...
class REM(nn.Module):
    ...
    def __init__(self, in_channels,base_channels=8):
        ...
        self.conv0=Conv2d(in_channels,base_channels*2,3,stride=1,padding=1)
        self.conv1=Conv2d(base_channels*2,base_channels*4,3,stride=1,padding=1)
        self.conv2=Conv2d(base_channels*4,base_channels*4,3,stride=1,padding=1)
        self.conv3=Conv2d(base_channels*4,base_channels*2,3,stride=1,padding=1)
        self.conv4=Conv2d(base_channels*2,1,3,stride=1,padding=1)
...
def depth_regression(p, depth_values):
    ...
    #show_prob(depth_values,p,100,50)

    return depth

def ddr_loss(inputs,refine_depth_map, depth_gt_ms, mask_ms, **kwargs):
    ...
    for (stage_inputs, stage_key) in [(inputs[k], k) for k in inputs.keys() if "stage" in k]:
        ...
        if stage_key=="stage1":
            depth_loss_ddr=F.smooth_l1_loss(refine_depth_map[stage_idx][mask], depth_gt[mask], reduction='mean')
            total_loss += 3 * depth_loss_ddr
...
def get_normal(pro_volume):

    prob_sum=torch.sum(pro_volume,1).unsqueeze(1)
    pro_volume_out=pro_volume/prob_sum
    return pro_volume_out

def Loss_strategy(pro_volume,depth_values,depth_min,depth_max):
    pro_volume=pro_volume.detach()
    depth_values=depth_values.detach()
    temp=depth_values-depth_max
    temp=temp.clamp(max=0)
    depth_values_clamp=temp+depth_max

    temp=depth_values_clamp-depth_min
    temp=temp.clamp(min=0)
    depth_values_clamp=temp+depth_min

    pro_volume_clamp=torch.where(depth_values!=depth_values_clamp,torch.full_like(pro_volume,0),pro_volume)
    pro_volume_clamp=pro_volume_clamp+eps
    pro_volume_normal=get_normal(pro_volume_clamp)

    depth=depth_regression(pro_volume_normal,depth_values_clamp)
    return depth



def get_cur_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel, uncertainty_map,min_depth,max_depth):
    #shape, (B, H, W)
    #cur_depth: (B, H, W)
    #return depth_range_values: (B, D, H, W)
    cur_depth_min = (cur_depth - ndepth / 2*uncertainty_map* depth_inteval_pixel*1.5)  # (B, H, W)
    cur_depth_max = (cur_depth + ndepth / 2 * depth_inteval_pixel*uncertainty_map*1.5)
    if min_depth.dim()==1:
       cur_depth_min=cur_depth_min.clamp(min=min_depth[0].item())
       cur_depth_max=cur_depth_max.clamp(max=max_depth[0].item())
    ...
    return depth_range_samples,cur_depth_min,cur_depth_max,new_interval


def get_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel, device, dtype, shape,uncertainty_map,min_depth,max_depth):
    #shape: (B, H, W)
    ...
    if cur_depth.dim() == 2:
        ...
        depth_range_samples,cur_depth_min,cur_depth_max,new_interval = get_cur_depth_range_samples(cur_depth, ndepth, depth_inteval_pixel,uncertainty_map,min_depth,max_depth)

    return depth_range_samples,cur_depth_min,cur_depth_max
