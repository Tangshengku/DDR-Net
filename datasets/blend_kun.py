...
# the Blended dataset preprocessed by Shengkun Tang (only for training)
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            img_filename = os.path.join(self.datapath,
                                        '{}/blended_images/{:0>0}.jpg'.format(scan, vid))

            mask_filename_hr = os.path.join(self.datapath, '{}/blended_images/{:0>0}_mask.jpg'.format(scan, vid))
            depth_filename_hr = os.path.join(self.datapath, '{}/rendered_depth_maps/{:0>8}.pfm'.format(scan, vid))

            proj_mat_filename = os.path.join(self.datapath, '{}/cams/train/{:0>8}_cam.txt').format(scan, vid)
