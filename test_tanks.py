...
parser.add_argument('--testpath',default='../DTUdataset/dtu', help='testing data dir for some scenes')
...
parser.add_argument('--loadckpt', default='checkpoints_loss/model_000019.ckpt', help='load a specific checkpoint')
parser.add_argument('--outdir', default='outputs_stage1_tanks', help='output dir')
...
parser.add_argument('--max_h', type=int, default=2048, help='testing max h')
parser.add_argument('--max_w', type=int, default=2048, help='testing max w')
...
#filter
parser.add_argument('--conf', type=float, default=0.8, help='prob confidence')
...
parser.add_argument('--prob_threshold', type=float, default='0.999')
parser.add_argument('--disp_threshold', type=float, default='0.25')
parser.add_argument('--num_consistent', type=float, default='5')
...
def save_depth_ypy(testlist,scene_num):
    for scene in testlist:
        if (scene==scene_num):
            save_scene_depth([scene])
...
def save_scene_depth(testlist):
    ...
    # model
    model = DDR_Net(refine=False, ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            outputs,refine_depth_map = model(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
            end_time = time.time()
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                ...
                feature_filename = os.path.join(args.outdir, filename.format('feature', '.jpg'))
